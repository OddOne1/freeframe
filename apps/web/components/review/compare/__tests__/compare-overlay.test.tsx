...
/**
 * Both sides of the pair live in the URL (upstream issue #299).
 *
 * Upstream kept the LEFT version and both offsets in the URL but the RIGHT one
 * only in the review store. A reload or a pasted compare link therefore
 * restored the left version and the trims faithfully and silently swapped the
 * right pane to whatever the store held — the wrong pair, with the previous
 * pair's offsets still applied, looking like a successful restore.
 */
describe('right pane version round-trips through the URL (#299)', () => {
  /** The trigger button shows "v<n>" for the version that pane is showing. */
  const shown = (testId: string) =>
    within(screen.getByTestId(testId)).getByRole('button').textContent?.trim()

  const renderWith = (rightVersionNumber: number, all = [1, 2, 3]) =>
    render(
      <CompareOverlay
        asset={asset}
        versions={all.map((n) => makeVersion(n))}
        rightVersion={makeVersion(rightVersionNumber)}
        onClose={vi.fn()}
      />,
    )

  it('restores the right version named in the URL, not the store-supplied one', () => {
    // The link says compare v-1 against v-2. The prop — the page's current
    // version, i.e. the asset's newest — says v-3.
    searchParamsString = 'compare=v-1&compareRight=v-2'
    renderWith(3)
    expect(shown('compare-select-b')).toBe('v2')
    expect(shown('compare-select-a')).toBe('v1')
  })

  it('falls back to the supplied version when the URL names none', () => {
    // Every link written before this fix, and the first open from the button.
    searchParamsString = 'compare=v-1'
    renderWith(3)
    expect(shown('compare-select-b')).toBe('v3')
  })

  it('ignores a right version the asset does not have', () => {
    // A hand-edited or stale link must not blank the pane.
    searchParamsString = 'compare=v-1&compareRight=v-nope'
    renderWith(3, [1, 3])
    expect(shown('compare-select-b')).toBe('v3')
  })

  it('writes the new right version to the URL when it is switched', () => {
    searchParamsString = 'compare=v-1&compareRight=v-3&offA=2&offB=5'
    renderWith(3)
    replace.mockClear()
    fireEvent.click(within(screen.getByTestId('compare-select-b')).getByRole('button'))
    fireEvent.click(screen.getByRole('option', { name: /v2/ }))
    const url = String(replace.mock.calls.at(-1)?.[0] ?? '')
    expect(url).toContain('compareRight=v-2')
    // The offsets belonged to the OLD pair and must not survive it — the exact
    // combination that made the original bug look like a working restore.
    expect(url).not.toContain('offA=')
    expect(url).not.toContain('offB=')
  })

  it('left falls back relative to the URL-restored right, not the supplied one', () => {
    // With right restored to v-2 from the URL, an unknown `compare` should pick
    // the nearest older version than v-2 — not than v-3.
    searchParamsString = 'compare=unknown&compareRight=v-2'
    renderWith(3)
    expect(shown('compare-select-a')).toBe('v1')
    expect(shown('compare-select-b')).toBe('v2')
  })
