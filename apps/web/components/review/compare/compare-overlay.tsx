...
// NOTE: upstream also passes `exportVersionId` to CommentPanel here, for its
// comment-EXPORT feature (CSV/EDL). This fork has no comment export, so the
// prop does not exist on our CommentPanel and is not passed — see the note
// at CommentPanelProps.
...
export function CompareOverlay({ asset, versions, rightVersion, onClose, canComment = true }: CompareOverlayProps) {
  ...
  // BOTH sides come from the URL (upstream #299). The left always did; the
  // right lived only in the review store, while offA/offB lived in the URL —
  // so a reload or a pasted compare link restored the left version and both
  // offsets faithfully and silently reset the right pane to the asset's
  // newest version. That is the wrong pair, with the previous pair's trims
  // still applied, and it looks like a successful restore.
  const compareParam = searchParams.get('compare')
  const compareRightParam = searchParams.get('compareRight')
  const rightFromUrl = ready.find((v) => v.id === compareRightParam) ?? null
  const right = rightFromUrl ?? rightVersion
  const fallbackLeft =
    [...ready].reverse().find((v) => v.version_number < right.version_number) ??
    ready.find((v) => v.id !== right.id) ?? null
  const left = ready.find((v) => v.id === compareParam) ?? fallbackLeft

  // Keep the page underneath in step with what the URL restored, so closing
  // compare lands on the version the right pane was actually showing rather
  // than the one the store happened to hold. Only when they disagree — an
  // unconditional write would loop against setCurrentVersion's own re-render.
  React.useEffect(() => {
    if (rightFromUrl && rightFromUrl.id !== rightVersion.id) setCurrentVersion(rightFromUrl)
  ...
  // The SCRUBBER's frame-step is one number by nature: it steps along the
  // shared timeline, which has no per-side frame rate. Kept blended for that.
  ...
  const transport = useSyncedTransport({
    ...
    // PER SIDE, not the blended value above (upstream #183). Each pane's
    // paused drift tolerance has to be ~one of its OWN frames; feeding both
    // a single blended rate gives the mismatched side a tolerance that is
    // not a real frame of its media, and every paused tick re-seeks a
    // decoder that was already correct.
  ...
  const handleSwitchRight = React.useCallback(
    (v: AssetVersion) => {
      // The right pane's version now goes in the URL alongside its offsets
      // (#299), so a reload restores the same pair. The store is still
      // written so the page underneath follows, but the URL is the record.
      // Batched: the store write is synchronous and router.replace is
      // deferred, so updating them separately renders the NEW version with
      // the OLD offset still applied for a commit or two.
      React.startTransition(() => {
        writeParams((p) => { p.set('compareRight', v.id); p.delete('offA'); p.delete('offB') })
        setCurrentVersion(v)
        setAnnotationB(null)
