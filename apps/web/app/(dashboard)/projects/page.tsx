...
import type { Project, ProjectType, AdminProject, User } from "@/types";
...
function AdminAllProjectsSection() {
  ...
  return (
    <div className="space-y-4 pt-4 border-t border-border">
      <div className="flex items-center gap-2">
        ...
        <span className="flex h-5 min-w-5 items-center justify-center rounded-full bg-bg-tertiary px-1.5 text-[10px] font-medium text-text-tertiary">
          {projectsResp?.length ?? 0}
        </span>
      </div>
      <p className="text-xs text-text-tertiary">
        Every project on the platform, including ones you&apos;re not a member
        of. You can manage any of them from here, but you&apos;ll need to
        join a project to see its actual contents.
      </p>
...
export default function ProjectsPage() {
  ...
  // "My Projects" = projects where I'm a Project Admin (role=owner
  // membership), not just the ones I literally created — a Project Admin
  // added later by someone else should land here too, not in "Shared
  // with Me", since they get full management rights on it.
  ...
  const publicProjects = React.useMemo(
    () => (projects ?? []).filter((p) => p.is_public && !p.role),
    [projects],
  );
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      {user?.is_superadmin && <AdminAllProjectsSection />}
    </div>
  );
}
